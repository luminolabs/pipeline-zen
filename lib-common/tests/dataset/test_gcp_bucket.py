...
def test_bucket_provider_init(valid_bucket_provider):
    ...
    assert valid_bucket_provider.url == "gs://lum-pipeline-zen-jobs-us/datasets/user123/dataset.jsonl"
...
def test_bucket_provider_fetch_valid_bucket(valid_bucket_provider, mock_logger):
    ...
    with patch('common.dataset.gcp_bucket.download_object') as mock_download:
        ...
        mock_download.assert_called_once_with(
            'lum-pipeline-zen-jobs-us',
            'datasets/user123/dataset.jsonl',
            valid_bucket_provider.local_path
        )

        # Verify log message
        mock_logger.info.assert_called_once_with(
            'Downloading GCP bucket dataset: gs://lum-pipeline-zen-jobs-us/datasets/user123/dataset.jsonl'
        )
...
def test_bucket_provider_fetch_invalid_bucket(invalid_bucket_provider):
    ...
    with pytest.raises(ValueError, match=r'Upload datasets to `gs://lum-pipeline-zen-jobs-us/datasets/<user_id>` only'):
...
def test_bucket_provider_with_different_paths(mock_logger):
    ...
    test_cases = [
        "gs://lum-pipeline-zen-jobs-us/datasets/user123/data.jsonl",
        "gs://lum-pipeline-zen-jobs-us/datasets/user123/subfolder/data.jsonl",
        "gs://lum-pipeline-zen-jobs-us/datasets/user123/deep/nested/data.jsonl"
    ]
