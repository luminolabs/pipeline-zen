...
resource "google_pubsub_subscription" "pipeline_zen_jobs_start" {
  for_each = toset(concat(tolist(keys(local.machine_types)),
      var.environment == "dev" ? ["1xlocal", "2xlocal"] : []))
...
resource "google_pubsub_topic" "pipeline_zen_jobs_heartbeats" {
  for_each = toset(concat(["pipeline-zen-jobs-heartbeats"],
    var.environment == "dev" ? ["pipeline-zen-jobs-heartbeats-local"] : []))
...
resource "google_pubsub_topic" "pipeline_zen_jobs_meta" {
  for_each = toset(concat(["pipeline-zen-jobs-meta"],
    var.environment == "dev" ? ["pipeline-zen-jobs-meta-local"] : []))
